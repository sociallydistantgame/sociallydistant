...
{
	public sealed class SshConsole : ITextConsole
	{
		private readonly ConcurrentQueue<ConsoleInputData> pendingKeys = new();
		private readonly ThreadSafeFifoBuffer pty = new();
		...
		public SshConsole(IDataWriter writer)
		{
			this.writer = writer;
		}
		...
		private void ControlCode(uint ascii)
		{
			...
			{
				case '\t': /* HT */
					this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Tab));
					return;
				case '\b': /* BS */
					this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Back));
					return;
				case '\r': /* CR */
					this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Enter));
					return;
				case 0x1b: /* ESC */
					this.CsiReset();
				...
				case 0x18: /* CAN */
					...
					break;
				case 0x7f: /* DEL */
					this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Delete));
					break;
		...
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Left));
		}

		private void HandleUpArrow()
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Up));
		}

		private void HandleDownArrow()
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Down));
		}

		private void HandleRightArrow()
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Right));
		}

		private void HandleHome()
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.Home));
		}

		private void HandleEnd()
		{
			this.pendingKeys.Enqueue(new ConsoleInputData(Keys.End));
		}
