		...
		INetworkConnection
	{
		...
		public async Task<PingResult> Ping(uint address, float timeuotInSeconds, bool acceptVoidPackets)
		{
			...
			if (!this.hostResolver.IsValidSubnet(address))
			{
				...
				if (!acceptVoidPackets)
					await Task.Delay((int) Math.Round(timeuotInSeconds * 1000));
			...
			bool receivedPong = await Task.WhenAny(pingTask, Task.Delay((int) Math.Round(timeuotInSeconds * 1000))) == pingTask;
		...
		public IListener Listen(ushort port, ServerType serverType = ServerType.Netcat, SecurityLevel secLevel = SecurityLevel.Open)
		{
			if (port >= (ushort.MaxValue - MaximumOutboundConnections))
				Log.Warning("Creating a listener on port " + port + " for in-bound connections is generally not advised. You are taking up one of the " + MaximumOutboundConnections + " reserved ports for out-bound traffic.");
