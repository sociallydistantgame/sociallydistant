...
namespace SociallyDistant.GameplaySystems.Chat
{
	public sealed class ConversationScriptController : IConversationController
	{
		...
		{
			Log.Information($"[CONVERSATION SCRIPT/{Conversation.Id}] <New interaction> Player chose branch {definitionId}");
		...
		{
			Log.Information($"[CONVERSATION SCRIPT/{Conversation.Id}] <{profile.ChatUsername} says> {message}");
		...
		{
			Log.Information($"[CONVERSATION SCRIPT/{Conversation.Id}] <New branch defined> {id}");
		...
		{
			Log.Information($"[CONVERSATION SCRIPT/{Conversation.Id}] Waiting for the player to choose a branch...");
