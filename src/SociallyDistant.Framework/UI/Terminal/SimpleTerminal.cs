...
public class SimpleTerminal
{
    ...
    {
        /* 0x41 - 0x7e */ '↑', '↓', '→', '←', '█', '▚', '☃', /* A - G */ '\0', '\0', '\0', '\0', '\0', '\0', '\0', '\0', /* H - O */ '\0', '\0', '\0', '\0', '\0', '\0', '\0', '\0', /* P - W */ '\0', '\0', '\0', '\0', '\0', '\0', '\0', ' ', /* X - _ */ '◆', '▒', '␉', '␌', '␍', '␊', '°', '±', /* ` - g */ '␤', '␋', '┘', '┐', '┌', '└', '┼', '⎺', /* h - o */ '⎻', '─', '⎼', '⎽', '├', '┤', '┴', '┬', /* p - w */ '│', '≤', '≥', 'π', '≠', '£', '·'
    };
    ...
    public void MouseUp(MouseButton button, float x, float y)
    {
        ...
        switch (button)
        {
            case MouseButton.Left:
            {
                ...
                this.ExtendSelection(col, row, SelectionType.SEL_REGULAR, !this.isSelecting);
                break;
            }
        }
    }
    ...
    public void TerminalNew(int cols, int rows)
    {
        this.term = new Term();
    ...
    private void TtyUpdate()
    {
        ...
        while ((ret = this.TtyRead()) > 0)
        {
            ...
            this.timeout = (this.maxLatency - (this.now - this.trigger)) / this.maxLatency * this.minLatency;
            if (this.timeout > 0)
                return;
        }
    }
    ...
    {
        int alt = this.IS_SET(TermMode.MODE_ALTSCREEN)
            ? 1
            : 0;
    ...
    {
        this.MoveTo(x, y + ((this.term.c.state & CursorState.CURSOR_ORIGIN) != 0
            ? this.term.top
            : 0));
    }
    ...
    private void InsertBlank(int n)
    {
        ...
        if (size > 0)
        {
            ...
            ref Line line = ref this.term.line[this.term.c.y];
    ...
    private void PutChar(char u)
    {
        unsafe
        {
            ...
            if (this.IS_SET(TermMode.MODE_INSERT) && this.term.c.x + width < this.term.col)
                Array.Copy(this.term.line[this.term.c.y].glyphs, this.term.c.x, this.term.line[this.term.c.y].glyphs, this.term.c.x + width, width);
            ...
            {
                this.term.wrapcwidth[this.IS_SET(TermMode.MODE_ALTSCREEN)
                    ? 1
                    : 0] = width;
                this.term.c.state |= CursorState.CURSOR_WRAPNEXT;
            }
    ...
    )
    {
		 ...
		 */
        if (this.term.trantbl[this.term.charset] == Charset.CS_GRAPHIC0 && Between(u, 0x41, 0x7e) && vt100_0[u - 0x41] != '\0')
            u = vt100_0[u - 0x41];
        ;
    ...
    private void ControlCode(uint ascii)
    {
        switch (ascii)
        {
            ...
            case '\n': /* LF */
                ...
                this.NewLine(this.IS_SET(TermMode.MODE_CRLF)
                    ? 1
                    : 0, 1);
                return;
            case '\a': /* BEL */
            ...
            fixed (int* a = args)
            {
                ...
                for (lim = ptr + narg; ptr < lim; ptr++)
			      ...
			      codes. */
                                break;
                            default:
                                Log.Error($"erresc: unknown private set/reset mode {*ptr}");
                                break;
                        }
                    else
                        switch (*ptr)
                        {
                            ...
                            case 12: /* SRM -- Send/Receive */
                                SetTerminalMode(ref this.term.mode, set == 0
                                    ? 1
                                    : 0, TermMode.MODE_ECHO);
                                break;
                            ...
                            case 1047: /* swap screen, clearing alternate screen */
                                ...
                                break;
                            default:
                                Log.Error($"erresc: unknown set/reset mode {*ptr}\n");
                                break;
                        }
            }
        }
    ...
    private void SetAttr(int[] attr, int l)
    {
        ...
        for (i = 0; i < l; i++)
            ...
            {
                case 0:
                    this.term.c.attr.mode &= ~(GlyphAttribute.ATTR_BOLD | GlyphAttribute.ATTR_FAINT | GlyphAttribute.ATTR_ITALIC | GlyphAttribute.ATTR_UNDERLINE | GlyphAttribute.ATTR_BLINK | GlyphAttribute.ATTR_REVERSE | GlyphAttribute.ATTR_INVISIBLE | GlyphAttribute.ATTR_STRUCK);
                ...
                case 38:
                {
                    ...
                    switch (fmt)
                    {
                        case 2:
                            ...
                            if (i + 2 >= l)
                            {
                                Log.Error("Malformed RGB foreground color sequence");
                                CsiDump();
                                return;
                            }
                            ...
                            break;
                        default:
                            Log.Error($"Color format {fmt} not supported");
                            this.CsiDump();
                            return;
                    }
                ...
                default:
                    ...
                    {
                        Log.Error($"erresc(default): gfx attr {attr[i]} unknown\n");
                        this.CsiDump();
                    }
    ...
    private void CsiParse()
    {
        unsafe
        {
            fixed (byte* p = this.csiescseq.buf)
            {
                ...
                this.csiescseq.mode[1] = (byte)(p2 < p + this.csiescseq.len
                    ? *p2
                    : 0);
            }
        }
    ...
    private void CsiHandle()
    {
        unsafe
        {
            ...
            switch ((char)this.csiescseq.mode[0])
            {
                default:
                    unknown:
                    Log.Error("unknown csi");
                    this.CsiDump();
                    break;
                case '@': /* ICH -- Insert <n> blank char */
                    this.csiescseq.arg[0] = this.csiescseq.arg[0] == 0
                        ? 1
                        : this.csiescseq.arg[0];
                ...
                case 'f': /* HVP */
                    this.csiescseq.arg[0] = this.csiescseq.arg[0] == 0
                        ? 1
                        : this.csiescseq.arg[0];
                    this.csiescseq.arg[1] = this.csiescseq.arg[1] == 0
                        ? 1
                        : this.csiescseq.arg[1];
                ...
                case 'J': /* ED -- Clear screen */
                    switch (this.csiescseq.arg[0])
                    {
                        ...
                        case 2: /* all */
                            ...
                            /* alacritty does this: */
                            for (n = this.term.row - 1; n >= 0 && this.LineLength(ref this.term.line[n]) == 0; n--) ;
                            if (n >= 0) this.ScrollUp(0, this.term.row - 1, n + 1, ScrollMode.SCROLL_SAVEHIST);
                            this.ScrollUp(0, this.term.row - 1, this.term.row - n - 1, ScrollMode.SCROLL_NOSAVEHIST);
                            break;
                        default:
                            goto unknown;
                    }
    ...
    public string GetSelection()
    {
        ...
        {
            ref Line line = ref this.Tline(y);
            ...
            var xtemp = 0;
            if (this.sel.type == SelectionType.SEL_RECTANGULAR)
            {
                xtemp = this.sel.nb.x;
            ...
            {
                xtemp = this.sel.nb.y == y
                    ? this.sel.nb.x
                    : 0;
                gp = ref line.glyphs[xtemp];
                lastx = this.sel.ne.y == y
                    ? this.sel.ne.x
                    : this.term.col - 1;
            }
			 ...
			 */
            if ((y < this.sel.ne.y || lastx >= linelen) && ((line.glyphs[xtemp2].mode & GlyphAttribute.ATTR_WRAP) == 0 || this.sel.type == SelectionType.SEL_RECTANGULAR))
                str.AppendLine();
        }
    ...
    {
        if (this.sel.ob.x == -1 || this.sel.mode == SelectionMode.SEL_EMPTY || this.sel.alt != this.IS_SET(TermMode.MODE_ALTSCREEN) || this.sel.nb.y > y2 || this.sel.ne.y < y1)
            return false;

        return this.sel.type == SelectionType.SEL_RECTANGULAR
            ? this.sel.nb.x <= x2 && this.sel.ne.x >= x1
            : (this.sel.nb.y != y2 || this.sel.nb.x <= x2) && (this.sel.ne.y != y1 || this.sel.ne.x >= x1);
    }
    ...
    )
    {
        ...
        var oldsby = 0;
        var oldsey = 0;
        var oldtype = default(SelectionType);
        ...
        if (oldey != this.sel.oe.y || oldex != this.sel.oe.x || oldtype != this.sel.type || this.sel.mode == SelectionMode.SEL_EMPTY)
            this.SetDirty(Math.Min(this.sel.nb.y, oldsby), Math.Max(this.sel.ne.y, oldsey));

        this.sel.mode = done
            ? SelectionMode.SEL_IDLE
            : SelectionMode.SEL_READY;
    }
    ...
    private void StartSelection(int col, int row, SelectionSnap snap)
    {
        ...
        this.sel.type = SelectionType.SEL_REGULAR;
        ...
        switch (this.sel.snap)
        {
            case SelectionSnap.SNAP_WORD:
                ...
                for (;;)
                {
                    ...
                    if ((gp.mode & GlyphAttribute.ATTR_WDUMMY) == 0 && (delim != prevdelim || (delim && !(gp.character == ' ' && prevgp.character == ' '))))
                        break;
    ...
    private void NormalizeSelection()
    {
        ...
        if (this.sel.type == SelectionType.SEL_REGULAR && this.sel.ob.y != this.sel.oe.y)
        {
            this.sel.nb.x = this.sel.ob.y < this.sel.oe.y
                ? this.sel.ob.x
                : this.sel.oe.x;
            this.sel.ne.x = this.sel.ob.y < this.sel.oe.y
                ? this.sel.oe.x
                : this.sel.ob.x;
        }
        else
    ...
    private void Reflow(int col, int row)
    {
        ...
        var nlines = 0;
        Line[] buf = Array.Empty<Line>();
        var line = default(Line);

        /* y coordinate of cursor line end */
        for (oce = this.term.c.y; oce < this.term.row - 1 && this.IsWrapped(ref this.term.line[oce]); oce++) ;
    ...
    )
    {
        ...
        if (savehist)
        {
            for (i = 0; i < n; i++)
            {
                ...
                Line temp = this.term.hist[this.term.histi];
        ...
        for (i = top; i <= bot - n; i++)
        {
            ref Line temp = ref this.term.line[i];
            ref Line temp2 = ref this.term.line[i + n];
    ...
    private ref Line Tlineabs(int y)
    {
        return ref y < 0
    ...
    {
        if ((this.term.c.state & CursorState.CURSOR_WRAPNEXT) != 0 && this.term.c.x + this.term.wrapcwidth[alt
                ? 1
                : 0] < col)
        {
            this.term.c.x += this.term.wrapcwidth[alt
                ? 1
                : 0];
            this.term.c.state &= ~CursorState.CURSOR_WRAPNEXT;
        }
    ...
    private bool IsWrapped(ref Line line)
    {
    ...
    private string GetGlyphs(ref Line line, int gp, int lgp)
    {
    ...
    private string GetLine(ref Line line)
    {
        var fgp = 0;
    ...
    private int LineLength(ref Line line)
    {
    ...
    public static T[] XRealloc<T>(ref T[] array, int size)
    {
        ...
        {
            Log.Error("Whoa whoa whoa! Something needs to be restitched immediately. We ran out of RAM.");
        }
