		...
		private void HandleInput(ConsoleInputData input)
		{
			...
			if (console.IsInteractive)
			{
				switch (input.KeyCode)
				{
					...
					case Keys.Tab when !input.HasModifiers && !UsePasswordChars:
						this.InsertAutoComplete();
						break;
					
					case Keys.Up when !input.HasModifiers && !UsePasswordChars:
						SelectPreviousCompletion();
						break;
					case Keys.Down when !input.HasModifiers && !UsePasswordChars:
						SelectNextCompletion();
						break;
					
					case Keys.Left when input.Modifiers.HasFlag(KeyModifiers.Shift):
						ExpandSelection(-1);
						break;
					case Keys.Right when input.Modifiers.HasFlag(KeyModifiers.Shift):
						ExpandSelection(1);
						break;
					case Keys.Home when input.Modifiers.HasFlag(KeyModifiers.Shift):
						ExpandSelection(-lineBuilder.Length);
						break;
					case Keys.End when input.Modifiers.HasFlag(KeyModifiers.Shift):
						ExpandSelection(lineBuilder.Length);
						break;
				}
			}
